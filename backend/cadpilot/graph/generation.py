...
def build_generation_graph(standards: Standards | None = None):
    ...
    def _assemble(state: GenerationState, topo: TopologyProposal, instr: InstrumentationProposal) -> EngineeringModel:
        ...
        model.metadata["sizing"] = state["equipment_proposal"].sizing  # mass balance → number of units
        from ..engine import basis as design_basis
        model.metadata["basis"] = design_basis.extract(state["source"], state["intent"].basis_approvals).model_dump()
        return model
