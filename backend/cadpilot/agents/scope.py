...
def _rule_findings(source: SourceData, standards: Standards) -> list[ModuleFinding]:
    ...
    for mid, m in standards.modules.items():
        ...
        used: set[str] = set()
        for s in stages:
            hit = next((r for r in keyword_matches(s, rows) if r.ref not in used), None)
            if hit is None:
                continue
            used.add(hit.ref)
        ...
        n, total = len(evidence), len(stages)
...
def detect(source: SourceData, standards: Standards, memory: dict | None = None) -> ScopeDetection:
    findings = _rule_findings(source, standards)
    ai = _ai_review(source, standards, findings, memory)
